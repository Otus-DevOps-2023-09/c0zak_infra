...
Основное ДЗ:
        Всё сделано.
        ui пришлось делать на убунте, на алпине падает(
...
Доп. задание:
        Система изначально строилась для автодеплоя. Так что сначала создаём пакером болванку, затем её раскатываем терраформом. Они оба используют ансибл.
        По итогу создаётся кластер из двух машин, один мастер и один воркер.
        Выполнение из каталлога kubernetes:
                # packer build -var-file=./packer/variables.json ./packer/prepared_k8s.json
        Затем идём в каталог terraform и пишем полученный id образа в файл terraform.tfvars, image_id
        Далее из каталога terraform вызываем:
                # terrafrom apply
        По итогу работы получим готовый кластер.
...
Линк на DockerHub:
https://hub.docker.com/u/ublin

Основное ДЗ:
        Всё потыкано.

Доп. задание по монге:
        Добавлен экспортер от битнами, и джоб для прометея. Версия выбрана та, что может в монго 3.2

Доп. задание по blackbox:
        Добавлен экспортёр от битнами, в прометее джоб по 200ым кодам, и сами метрики blackbox.

Доп. задание по Makefile:
        Сам Makefile уложен в каталоге docker в корне.
        Заданы таргеты:
                build - сборка всех образов
                push - пушинг на dockerhub
                clean - полный вынос всех контейнеров и образов из системы
        Остальные таргеты позволяют управлять конкретными сервисами.
...
В каталоге gitlab-ci лежит файлик docker-compose,yml, но по факту на него не опираемся, это копия j2 шаблона.
Система сразу строилась для автодеплоя.

Доп. задание по автодеплою:
        Алгоритм (от каталога gitlab-ci):
                1) Выполняем packer build -var-file=./packer/variables.json ./packer/prepared_docker.json
                        Этот запрос создаст базовую ВМ на убунте 18, затем дёрнет ansible роль по установке docker.
                2) Переходим в каталог terraform, назначаем в файле переменных значение image_id полученное из пакера, остальным тоже проставляем значения.
                3) Выполняем terraform apply
                        По завершению получаем готовый инстанс гитлаба, также будет выведен IP для подключения и пароль юзера root. На текущих настройках около 6 минут деплой займёт. В процессе будет использована ansible роль gitlab

Доп. задание по запуску контейнера с reddit в динамическом окружении:
        Всё описано в файле .gitlab-ci.yml в корне, стадия build_job

Доп. задание по раннеру:
        В каталоге gitlab-ci/runner есть compose файл для подъёма раннера, и скрипт для его регистрации. Достаточно запустить скрипт, только сначала в него надо вписать внешний IP и токен для раннера.
...
3 варика на самом деле:
        1) Через параметр project-name
        2) Через переменную COMPOSE_PROJECT_NAME (я так и делаю)
        3) Непосредственно в compose файле, но это только для версии 3.8 и старше, параметр name на верхнем уровне

Доп. задание:
        Написан compose файл, который хватает файлы приложений из каталогов на хосте через volume,
        Параметры запуска переданы через command
...
Доп. задание:
        1) Различие между контейнером и образом расписано в docker-1.log
           Хочу заметить, что основывать понимание фундаментального определения на выводе команды - это бред.

        2) В каталоге инфра создан проект развёртывания.
           packer - есть два конфига, base - для голого образа ОС, full - полновесный bake образ.
           terraform - понимает инстанс. Переменная instance_count определяет количество инстансов. По id образа можно выбрать из того, что пакер насобирал.
           ansible - окей, множим сущности, хотя всё можно решить в один плейбук. Сделано две роли, docker - ставит докер, reddit - запускает контейнер. Инвентори динамический, берётся из S3 бакета.
...
Результаты работы
Общее ДЗ:
        Выполнено, но пришлось капитально изголяться. В частности править исходники молекулы. Ваши домашки нихрена не вяжутся с актуальными версиями продуктов, а подходящие версии - с питоном современным.
        Переделывайте.

Доп. задание:
        Дополните конфигурацию Vagrant для корректной работы проксирования приложения с помощью nginx

        В Vagrantfile добавлена секция ansible.extra_vars
        ---
        ...
        Подселил в соседнюю репу, всё взошло.
...
Результаты работы
Общее ДЗ:
        Все роли созданы и протестированы.

Доп. задание:
        В terraform добавлен label stage/prod для машин, а в плагин yc_compute - фильтрация хостов по этому label, в зависимости от окружения.

Доп. задание 2:
        Так как мы не используем TravisCI, а GitHub Actions, то задача решена частично.
        В каталоге play-travis лежат скрипты проверок, в корне есть .travis.yml, в нём описан порядок вызовов. Но все проверки проведены локально.
        Плюс не стал делать бейдж, так как для этого уже нужно давать доступ тревису.
...
Доп. задание:
        ...
        В ансибл добавлен плагин yc_compute. (https://github.com/st8f/community.general/tree/yc_compute)
        В каталоге создан динамический инвентори, yc_compute.yaml
        А yaml он, потому что ваш тупенький тест гребёт все yml, не просекая что там описание плагина...
